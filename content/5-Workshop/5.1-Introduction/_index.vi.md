...
Để có cái nhìn tổng quan về cách các dịch vụ AWS tương tác với nhau trong dự án game Cloud Battleship Arena, hãy tham khảo sơ đồ kiến trúc dưới đây:

![Sơ đồ kiến trúc AWS](/images/5-Workshop/5.1-Introduction/AWS_Architecture.png)

Sơ đồ mô tả luồng xử lý từ lúc người dùng truy cập giao diện qua CloudFront, xác thực qua Cognito, kết nối thời gian thực qua WebSocket API Gateway đến việc xử lý logic qua Lambda và lưu trữ dữ liệu tại DynamoDB/S3.
...
| **Amazon CloudFront** | Cổng vào duy nhất (Single Entry Point), phân phối nội dung toàn cầu và proxy/security cho REST & WebSocket API qua WAF. |
